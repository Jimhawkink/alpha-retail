        ...
        try {
            ...
            for (const [outletId, feats] of Object.entries(outletFeatures)) {
                await supabase.from('license_settings').upsert(
                    { setting_key: `outlet_features_${outletId}`, setting_value: JSON.stringify(feats) },
                    { onConflict: 'setting_key' }
                );
            }
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden fu">
                ...
                {activeTab === 'system' && (
                    <div className="p-6 space-y-6">
                        ...
                        {/* ─── Granular Feature Toggles per outlet (SuperAdmin only) ─── */}
                        {isSuperAdmin && outlets.length > 0 && (() => {
                            const FEATURES = [
                                { key: 'valuation_cards',   label: '📊 Stock Valuation Cards',      sub: 'Premium glassmorphic cards + 5-card intelligence' },
                                { key: 'retail_profit',     label: '🏷️ Retail Profit Analytics',    sub: 'Retail profit/pc card in Add Product modal' },
                                { key: 'wholesale_profit',  label: '🤝 Wholesale Profit Analytics',  sub: 'WS profit, profit/bag, margin cards in Add Product modal' },
                            ];
                            return (
                                <div className="p-5 rounded-2xl border-2 border-emerald-200 bg-emerald-50/40">
                                    <div className="flex items-center gap-2 mb-4">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm text-emerald-800">Premium Feature Control — Per Outlet</p>
                                            <p className="text-[11px] text-emerald-600">Individually enable/disable each premium feature per outlet</p>
                                        </div>
                                    ...
                                    <div className="space-y-3">
                                        {outlets.map(outlet => {
                                            const feats = outletFeatures[outlet.outlet_id] ?? [];
                                            return (
                                                <div key={outlet.outlet_id} className="bg-white rounded-xl border border-emerald-100 shadow-sm overflow-hidden">
                                                    <div className="flex items-center gap-3 px-4 py-2.5 border-b border-emerald-50">
                                                        <div className={`w-2 h-2 rounded-full ${feats.length > 0 ? 'bg-emerald-500' : 'bg-gray-300'}`} />
                                                        ...
                                                        <span className={`ml-auto text-[10px] font-bold px-2 py-0.5 rounded-full ${feats.length > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>
                                                            {feats.length === 0 ? '🔒 No features' : `✅ ${feats.length} feature${feats.length > 1 ? 's' : ''} enabled`}
                                                        </span>
                                                    </div>
                                                            ...
                                                            return (
                                                                <label key={f.key} className="flex items-center gap-3 cursor-pointer group">
                                                                    <input
                                                                        ...
                                                                        onChange={() => {
                                                                            const next = enabled
                                                                                ? feats.filter((x: string) => x !== f.key)
                                                                                : [...feats, f.key];
                                                                            setOutletFeatures(prev => ({ ...prev, [outlet.outlet_id]: next }));
                                                                        }}
                                                                        className="w-4 h-4 rounded accent-emerald-600"
                                                                    />
                                                                    <div>
                                                                        <p className={`text-xs font-bold ${enabled ? 'text-emerald-700' : 'text-gray-500'}`}>{f.label}</p>
                                    ...
                                    <p className="text-[11px] text-emerald-500 mt-3">💡 Check/uncheck features then click <strong>Save Changes</strong> above. Takes effect on next page load per outlet.</p>
                                </div>
                            );
                        })()}
